...
- [~] CH4 Implementation and Results → `docs/report/chapters/ch4.md` (§4.1 Intro, §4.2 Tools & Materials [Tables 4.1, 4.2], §4.3 Implementation Process [4.3.1–4.3.7] DRAFTED; §4.3.4 amended 2026-06-21 to add the objective, university-wide packing, off-site placeholder-room handling; §4.3.5 notes regeneration-replaces; §4.3.3 now covers run-list status scoping + analytics role-scoping. §4.4 Results, §4.5 Evaluation, §4.6 Partial Conclusion are headed placeholders awaiting the FET case-study run. Evaluation framing agreed: vs manual FET process + correctness verification + objectives scorecard + brief existing-tools callback.)
- [ ] CH5 Conclusion and Further Works (Further Works source notes accumulating in `docs/report/further_works_backlog.md` — currently: verified-email / invitation flow)
