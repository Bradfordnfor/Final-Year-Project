...
**Manual over-capacity relief (post-generation):** officer can **take a class out** of an over-capacity merged session into its own session (`split-class`: pick class + new slot + room), or **put a class in** to another same-course session (`merge-class`). split-class now also lands on an OCCUPIED hall when that hall already runs the SAME course with the SAME lecturer in that period — the class joins it, within the room's overflow allowance; an occupied hall of a different course/lecturer, or one that would overflow, is rejected. Both held to generation's rules (no class double-booking; no overflow beyond allowance). UI = 3-tab entry dialog (Move | Take out | Put in).

**Other features built:** manual slot move with clash detection (click-to-edit dialog, 409 on clash shown inline); analytics endpoint (sessions/lecturer with click-through course breakdown, overcapacity count, room utilization) — NOT visible to officer; university_admin sees whole run, faculty_head sees only their faculty; PDF + CSV export (ReportLab) with optional class-scope filter (`class_ids=`); notifications on publish/review/approve/reject; **name-based bulk import** of lecturers via CSV (`name,email,faculty,department,password`; faculty/dept matched by name within the admin's university; password optional/auto-generated; per-row skip reasons; **two-step preview→confirm** via `?dry_run=true` — preview validates and lists would-create/would-skip without writing, commit on confirm; within-file duplicate emails flagged; a non-CSV/unreadable file returns a friendly 400, not a 500); university creation with admin credentials; typed-name delete protection; university-wide courses; class population editable; lecturer→course assignment; lecturer self-service availability (`/lecturers/me` auto-provisions profile); class groups management; shareable public link (`/#/public?run=<id>`); public + internal timetable with Faculty→Dept→Level class filter supporting partial scope (faculty-wide / dept-wide / single class); free-rooms-by-period panel; **public timetable download** (CSV/PDF via unauthenticated `/export/public/runs/{id}/...`, published runs only, honours the class filter); **My Account** dialog showing name+role (read-only) and editable email (`POST /auth/change-email`, unverified — invitation/verification is future work), with change-password behind a button. **Run-list visibility by role+status:** officer & university_admin see all states; faculty_head & lecturer see only under_review + published; drafts private to officer/admin. **Regeneration replaces** the previous result (clears entries/conflicts first). **Readiness check** also verifies the run's faculties hold at least one CLASS (an empty faculty otherwise passed via the university-wide courses it pulls into scope; the room-type item is suppressed when there are no classes, so the checklist points at the real gap). **Run deletion** from the run list (officer; plain confirm for draft/under_review/approved, typed-name confirm for published since there is no unpublish). **Saturday & Sunday time slots** supported: slot-creation UI offers Mon–Sun; grids show Mon–Fri always + Sat/Sun when those days have slots (`displayWeekDays` helper). Approval panel shows **faculty code** (e.g. FET), not the raw id.
...
- [x] Preliminary pages → `docs/report/front_matter.md` (DRAFTED 2026-06-22: cover/title identity, Certification, Dedication [template — personalise], Acknowledgement [draft — personalise], Abstract [~400 wds + keywords], List of Abbreviations). ToC / List of Tables / List of Figures to be auto-generated by build_report.py.
- [x] References (APA) → `docs/report/references.md` (DRAFTED 2026-06-22: 14 entries, alphabetical, matching CH1–CH4 citations. Note: CH4 §4.5.5 "UniTime or FET" changed to just "UniTime" to avoid FET-software vs FET-faculty ambiguity.)
- [x] build_report.py → final .docx (DONE 2026-06-22). `docs/report/build_report.py` assembles front_matter + ch1–5 + references into `Dissertation.docx` with UB formatting (A4, Times New Roman 16/14/13/12, 1.5 justified, 2.5/2cm margins, blue bordered cover, Roman prelims → Arabic body via 3 sections, Word ToC field, auto List of Tables/Figures from captions). Run: `backend/venv/Scripts/python build_report.py` (needs python-docx, installed in venv). Output gitignored. MANUAL STEPS BEFORE SUBMISSION: in Word, right-click the ToC → Update Field; render the 11 CH3 PlantUML diagrams + insert the 10 CH4 screenshots at their Figure captions; personalise Dedication/Acknowledgement; List of Tables/Figures currently have no page numbers (captions only).
