# References

> APA style. Entries are the works cited in Chapters 1–4, alphabetical by author.

Abramson, D. (1991). Constructing school timetables using simulated annealing: Sequential and parallel algorithms. *Management Science, 37*(1), 98–113.

Applied Software Consultants. (n.d.). *aSc Timetables* [Computer software]. Retrieved from https://www.asctimetables.com

Burke, E. K., & Petrovic, S. (2002). Recent research directions in automated timetabling. *European Journal of Operational Research, 140*(2), 266–280.

Carter, M. W., Laporte, G., & Lee, S. Y. (1996). Examination timetabling: Algorithmic strategies and applications. *Journal of the Operational Research Society, 47*(3), 373–383.

Colorni, A., Dorigo, M., & Maniezzo, V. (1990). Genetic algorithms and highly constrained problems: The time-table case. In H.-P. Schwefel & R. Männer (Eds.), *Parallel Problem Solving from Nature (PPSN I)*, Lecture Notes in Computer Science (Vol. 496, pp. 55–59). Springer.

de Werra, D. (1985). An introduction to timetabling. *European Journal of Operational Research, 19*(2), 151–162.

Even, S., Itai, A., & Shamir, A. (1976). On the complexity of timetable and multicommodity flow problems. *SIAM Journal on Computing, 5*(4), 691–703.

Lantiv. (n.d.). *Lantiv Timetabler* [Computer software]. Retrieved from https://lantiv.com

McCollum, B., Schaerf, A., Paechter, B., McMullan, P., Lewis, R., Parkes, A. J., Di Gaspero, L., Qu, R., & Burke, E. K. (2010). Setting the research agenda in automated timetabling: The second international timetabling competition. *INFORMS Journal on Computing, 22*(1), 120–130.

Mimosa Software. (n.d.). *Mimosa Scheduling Software* [Computer software]. Retrieved from https://www.mimosasoftware.com

Müller, T. (2005). *Constraint-based timetabling* [Doctoral dissertation, Charles University in Prague]. (UniTime). Retrieved from https://www.unitime.org

Perron, L., & Furnon, V. (2024). *OR-Tools* (Version 9.15) [Computer software]. Google. Retrieved from https://developers.google.com/optimization

Schaerf, A. (1999). A survey of automated timetabling. *Artificial Intelligence Review, 13*(2), 87–127.

Wren, A. (1996). Scheduling, timetabling and rostering — A special relationship? In E. K. Burke & P. Ross (Eds.), *Practice and Theory of Automated Timetabling (PATAT 1995)*, Lecture Notes in Computer Science (Vol. 1153, pp. 46–75). Springer.
