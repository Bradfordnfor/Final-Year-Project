...
Second, and more distinctively, it joins that solver to the two things the existing tools surveyed in Chapter 2 leave out: a multi-role review-and-approval workflow that establishes accountability for a timetable before it takes effect, and a student-facing distribution channel that needs no account. This combination of generation, coordination and distribution in one system is the part of the work that addresses the institutional problem rather than only the algorithmic one.
