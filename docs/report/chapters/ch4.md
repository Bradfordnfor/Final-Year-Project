...
| FastAPI | n/a | Web framework exposing the REST/JSON interface |
| Pydantic | 2 | Validation of request and response data |
| Uvicorn | n/a | ASGI server that runs the application |
| SQLAlchemy | 2.0 | Object-relational mapping to the database |
| Alembic | n/a | Database schema migrations |
| SQLite | 3 | Database (development configuration) |
| Google OR-Tools (CP-SAT) | 9.15 | Constraint solver at the heart of generation |
| python-jose | n/a | Issuing and verifying JWT authentication tokens |
| passlib + bcrypt | n/a | One-way hashing of passwords |
| ReportLab | n/a | Rendering timetables to PDF |
| pytest | n/a | Automated testing |
...
| GetX | n/a | State management, routing, dependency injection |
| Dio | n/a | HTTP client for calling the server |
| flutter\_secure\_storage | n/a | Secure storage of authentication tokens |
| fl\_chart | n/a | Charts on the analytics dashboard |
| google\_fonts (Inter) | n/a | Typography |
| flutter\_animate | n/a | Interface transitions |

### 4.2.4 Modelling and Documentation Tools

The diagrams of Chapter 3, namely the entity-relationship, use case, class, sequence, and activity diagrams, were drawn in **StarUML**, a dedicated UML modelling tool, and exported as images for inclusion in the report, so that each follows standard UML notation and could be revised as the design evolved alongside the code it describes.
...
A point worth recording is that the single `User` table carries not only the credentials and role of every account-holder but also the institutional links to university, faculty and department that later proved essential to scoping what each user may see and do. A faculty head, for instance, is an ordinary user whose `faculty_id` ties them to the one faculty they govern; that single field is what later distinguishes the head of Engineering from the head of any other faculty.
...
When the officer asks to generate, the system first checks that the run can actually yield a timetable. If something is missing, whether a course without a lecturer, a semester without time slots, or buildings without suitable rooms, generation is refused and a checklist names precisely what must be fixed (Figure 4.4), so the officer is never left guessing at an empty or failed result. Once every item is satisfied the same run is ready, and generation proceeds (Figure 4.5).
